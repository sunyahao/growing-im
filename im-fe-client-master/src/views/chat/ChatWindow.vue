...
export default {
  ...
  mounted() {
    ...
    if (IoService.socket == null)  {
      ...
      IoService.socket.on('/v1/im/person-other-messages1', function (data) {
        debugger;
        var message = {};
        ...
        message.body = body;
        if (data.reverseConversationId == that.query.conversationId) {
          that.messageList.push(message);
        }
      })

      // 在这里监听自己的消息
      IoService.socket.on('/v1/im/person-own-messages1', function (data) {
        debugger;
        var message = {};
        message.id = data.message.id;
        ...
        message.body = body;
        var id = that.activeConversation.id;
        debugger;
  ...
  methods: {
    ...
    activedConversation() {
      this.$http.post('/api/v1/conversations/active', {
