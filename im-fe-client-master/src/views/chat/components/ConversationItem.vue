...
export default {
  ...
  computed: {
    ...
    date() {
      var a = this.conversation.updatedAt;
      var i = parseInt(a);
      debugger;
      return moment(this.conversation.updatedAt).format("yyyy-MM-DD HH-mm-ss");
    }
  },
  watch: {},
  created() {
  ...
  methods: {
    ...
    onClick() {
      // 会话列表中只会有一个会话active为true，所以chatWindow是打开active为true的conversationItem
      var a = this.conversation;
      debugger;
      if (this.conversation.type == "user" || this.conversation.type == "chat") {
