    ...
    <div class="c-frame" :class="{ 'self-c': message.isMyself, 'other-c': !message.isMyself }">
      ...
      <div class="container">
        ...
        <div class="video" v-if="message.body.type === 'video'">
          ...
          <video :src="videoSrc" controls = "controls" width="200" height="100"></video>
        </div>
      </div>
      <div class="avatar" v-if="message.isMyself">
        <img :src="myselfHead" />
...
export default {
  ...
  created() {
    ...
    //   this.groupConversation.type === 'groupchat' &&
  ...
  computed: {
    ...
    videoSrc() {
      return this.message.body.url;
    },
    fromName() {
      if (this.message.isMyself) {
        console.log(this.message)
        return this.message.nickname + ' ' + moment(this.message.createdAt).format("YYYY-MM-DD hh:mm:ss");
      }
      return moment(this.message.createdAt).format("YYYY-MM-DD hh:mm:ss")  + ' ' + this.message.nickname;
    },
    fromUserInfo() {
      var a = this.groupConversation;
      debugger;
      if (this.groupConversation.type === 'chat') {
        return this.groupConversation.info;
      }
    ...
    config() {
      ...
      return {
        ...
        videoInit: true,
        fluid: true,
        url: "http://localhost:7001"+this.message.body.url
      };
    },
    friendHead() {
